...
export const paid = (loan: Loan) => Math.max(loan.payments.reduce((sum, p) => sum + p.amount, 0), (loan.fixedAmount || 0) * (loan.paidEmis || 0))
...
export function schedule(loan: Loan) {
  ...
  const start = new Date(loan.loanStartDate || new Date().toISOString().slice(0, 10))
  const end = new Date(loan.dueDate)
  const periods = loan.totalEmis || (loan.period === 'weekly' ? Math.max(1, Math.ceil((end.getTime() - start.getTime()) / 604800000)) : Math.max(1, (end.getFullYear() - start.getFullYear()) * 12 + end.getMonth() - start.getMonth() + 1))
  const amount = loan.payoffMode === 'fixed' ? (loan.fixedAmount || 0) : balance / periods
  for (let i = Math.max(1, loan.paidEmis + 1); i <= periods && balance > 0; i++) {
